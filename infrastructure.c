...
void MPvoice(char *msgbuf)
{

    int32_t dest_choice, bufflen,TX01;
    uint16_t *dest;
    char *loc;
    char posbuf[100];
    ...
    loc=MsgGetNextStr(msgbuf, true);
    if(!loc)
        return;

    dest_choice=atoi(loc);

    loc=MsgGetNextStr(loc, true);
    ...
    {
        TX01=0;
        dest= TXBufCtrl[TX01].TX_Ping;
    }
    else if(dest_choice==1)
    {
        TX01=0;
        dest= TXBufCtrl[TX01].TX_Pong;
    }
    ...
    if(TXBufCtrl[TX01].TX_Completed==NULL)
    {
        TXBufCtrl[TX01].TX_Completed=dest;
        TXBufCtrl[TX01].TX_index=0;
    }

    current = TXBufCtrl[TX01].TX_index;
    if(last[TX01] != current && lastlast[TX01] != current)
    {
        if(reg[SHADOW_DBG] & DBG_STREAM)
        {
            sprintf(posbuf,"-print %d",current);
            AddPayload(posbuf);
        }
        ...
        {
            TXBufCtrl[TX01].TX_correction = -1;
            if(reg[SHADOW_DBG] & DBG_STREAM)
                AddPayload("-print --");
        }
...
void MPstream(char *msgbuf)
{
    ...
    if(intent==ADCBufCtrl.converting)
        return;

    if(intent==0)
    {
        if(ADCBufCtrl.converting==2)
        {
            if(ADCBuf_convertCancel(adcBuf)!=ADCBuf_STATUS_SUCCESS)
            {
                AddPayload("-print ADC cancel failed");
                errorADC++;
            }
        }
        ADCBufCtrl.converting=0;
        if(lutCtrl.lutDelta > 0.0)
            MPsine("-sine 0");
        callback0Count = strClear(callback0Payload,msgMaxLen);
        ADCBufCtrl.RX_Completed=NULL;
        TXBufCtrl[0].TX_Completed=NULL;
        TXBufCtrl[0].TX_index=-1;
    }
    else if(intent==1 && ADCBufCtrl.converting==0)
    {
        GPIO_write(4,0);
        GPIO_write(5,1);
        callback0Count=-1;
        strClear(callback0Payload,msgMaxLen);
        strcpy(callback0Payload,"-audio");
        Timer_setPeriod(timer0,Timer_PERIOD_US,125);
        ADCBufCtrl.converting=1;
        ticker[15].delay=100;
        ticker[15].period=0;
        ticker[15].count=1;
        strcpy(ticker[15].function,"-stream 2");
    }
    else if(intent==2 && ADCBufCtrl.converting==1)
    {
        if(ADCBuf_convert(adcBuf,&ADCBufCtrl.conversion,1)!=ADCBuf_STATUS_SUCCESS)
        {
            AddPayload("-print ADC cancel failed");
            errorADC++;
        }
        else //==ADCBuf_STATUS_SUCCESS
        {
            ADCBufCtrl.converting=2;
            ADCBufCtrl.RX_Completed=NULL;
            TXBufCtrl[0].TX_Completed=NULL;
            TXBufCtrl[0].TX_index=-1;
            TXBufCtrl[0].TX_delay=DATADELAY;
            TXBufCtrl[0].TX_correction=0;
        }
    }
}

void MPnetUDP(char *msgbuf, int32_t binaryCount)
{   //send a payload out of Ethernet port using UDP
    char *loc;
    int32_t payloadnext;
    uint32_t gatekey;
    ...
    payloadnext=NetOutQ.payloadWriting +1;
    if(payloadnext >= PAYLOADCOUNT)
        payloadnext=0;
    if(payloadnext == NetOutQ.payloadReading)
    {
        GateSwi_leave(gateSwi2,gatekey);
        AddPayload("-print ERROR: UDP overflow");
        errorNet++;
        return;
    }
    memcpy(NetOutQ.payloads[NetOutQ.payloadWriting], loc, strlen(loc) + binaryCount + 1);
    NetOutQ.binaryCount[NetOutQ.payloadWriting]=binaryCount;

    NetOutQ.payloadWriting = payloadnext;
    GateSwi_leave(gateSwi2,gatekey);
    Semaphore_post(UDPOutSem);

}

void MPdial(char *msgbuf)
{
    char *loc;
...
//    uint32_t index;
    ...
    if(!loc)                                  //shut down call
    {
        sprintf(outputMsg, "-stream 0");      // Kill your call
        AddPayload(outputMsg);
        if(reg[SHADOW_DIAL0_IP]!=0)           //kill dial 0
        {
            sprintf(netbuf, "-netudp %d.%d.%d.%d:%d",
                    (uint8_t)(reg[SHADOW_DIAL0_IP] >> 24) & 0xFF, (uint8_t)(reg[SHADOW_DIAL0_IP] >> 16) & 0xFF,
                    (uint8_t)(reg[SHADOW_DIAL0_IP] >>  8) & 0xFF, (uint8_t)reg[SHADOW_DIAL0_IP] & 0xFF,
                    DEFAULT_NET_PORT);
            sprintf(outputMsg,"%s -stream 0", netbuf);
            AddPayload(outputMsg);
            sprintf(outputMsg,"%s -reg %d = #0", netbuf, SHADOW_DIAL0_IP);
            AddPayload(outputMsg);
            sprintf(outputMsg,"%s -reg %d = #0", netbuf, SHADOW_DIAL1_IP);
            AddPayload(outputMsg);
            reg[SHADOW_DIAL0_IP]=0;
        }

        if(reg[SHADOW_DIAL1_IP]!=0)
        {
            sprintf(netbuf, "-netudp %d.%d.%d.%d:%d",
                    (uint8_t)(reg[SHADOW_DIAL1_IP] >> 24) & 0xFF, (uint8_t)(reg[SHADOW_DIAL1_IP] >> 16) & 0xFF,
                    (uint8_t)(reg[SHADOW_DIAL1_IP] >>  8) & 0xFF, (uint8_t)reg[SHADOW_DIAL1_IP] & 0xFF,
                    DEFAULT_NET_PORT);
            sprintf(outputMsg,"%s -stream 0", netbuf);
            AddPayload(outputMsg);
            sprintf(outputMsg,"%s -reg %d = #0", netbuf, SHADOW_DIAL0_IP);
...
//        if(!loc)
//        {
//            AddPayload("-print Message missing required digits");
//            errDigits++;
//            return;
//        }
//        if(idfdigit(*loc) == 0)
//        {
//            AddPayload("-print Message '%s' missing required digits");
//            errDigits++;
//            return;
//        }
//        index=atoi(loc);
//        reg[SHADOWDIAL0_IP] = discoveries[index].IP_address;
//          count++;
//          loc=MsgGetNextStr(loc,true);
//          goto LOOK_FOR_SECOND;
//    }
    clientAddr.sin_addr.s_addr = 0;
    clientAddr.sin_port = DEFAULT_NET_PORT;

    loc = UDPParse(loc, &clientAddr, false);
    reg[SHADOW_DIAL0_IP]  = (clientAddr.sin_addr.s_addr & 0xFF)       << 24;
    reg[SHADOW_DIAL0_IP] |= (clientAddr.sin_addr.s_addr & 0xFF00)     << 8;
    reg[SHADOW_DIAL0_IP] |= (clientAddr.sin_addr.s_addr & 0xFF0000)   >> 8;
    reg[SHADOW_DIAL0_IP] |= (clientAddr.sin_addr.s_addr & 0xFF000000) >> 24;
    count++;
...
//        }
//        index=atoi(loc);
//        reg[SHADOWDIAL1_IP] = discoveries[index].IP_address;
//          count++;
//          loc=MsgGetNextStr(loc,false);
//          goto LOOK_FOR_SECOND;
//    }
    ...
    loc = UDPParse(loc, &clientAddr, true);
    reg[SHADOW_DIAL1_IP]  = (clientAddr.sin_addr.s_addr & 0xFF)       << 24;
    reg[SHADOW_DIAL1_IP] |= (clientAddr.sin_addr.s_addr & 0xFF00)     << 8;
    reg[SHADOW_DIAL1_IP] |= (clientAddr.sin_addr.s_addr & 0xFF0000)   >> 8;
    reg[SHADOW_DIAL1_IP] |= (clientAddr.sin_addr.s_addr & 0xFF000000) >> 24;
    count++;
...
//MAKE_DIAL_STRING:
    if(count ==1)
    {
        sprintf(netbuf, "-netudp %d.%d.%d.%d:%d",
               ...
               (uint8_t)(reg[SHADOW_DIAL0_IP] >>  8) & 0xFF, (uint8_t)reg[SHADOW_DIAL0_IP] & 0xFF,
               DEFAULT_NET_PORT);
        sprintf(outputMsg,"%s -reg %d = #%d", netbuf, SHADOW_DIAL0_IP, reg[SHADOW_NET_IP]);
        AddPayload(outputMsg);
        sprintf(outputMsg,"%s -script 49",netbuf);  // to run audio script in other board
        AddPayload(outputMsg);
        sprintf(outputMsg,"-script 49");            // to run audio script in my board
        AddPayload(outputMsg);
    }
    else if(count ==2)
    {
        ...
        sprintf(outputMsg,"%s -reg %d = #%d", netbuf, SHADOW_DIAL0_IP, reg[SHADOW_NET_IP]);
        AddPayload(outputMsg);
        sprintf(outputMsg,"%s -reg %d = #%d", netbuf, SHADOW_DIAL1_IP, reg[SHADOW_DIAL1_IP]);
        AddPayload(outputMsg);
        sprintf(outputMsg,"%s -script 49",netbuf);  // to run audio script in other board
        AddPayload(outputMsg);
        ...
        sprintf(outputMsg,"%s -reg %d = #%d", netbuf, SHADOW_DIAL0_IP, reg[SHADOW_NET_IP]);
        AddPayload(outputMsg);
        sprintf(outputMsg,"%s -reg %d = #%d", netbuf, SHADOW_DIAL1_IP, reg[SHADOW_DIAL0_IP]);
        AddPayload(outputMsg);
        sprintf(outputMsg,"%s -script 49",netbuf);  // to run audio script in other board
        AddPayload(outputMsg);
...
PRINT_ADDR:
    if(reg[SHADOW_DIAL0_IP] !=0)
    {
        sprintf(outputMsg, "-print ADDRESS DIAL 0 : %d.%d.%d.%d : %d",
            (uint8_t)(reg[SHADOW_DIAL0_IP] >> 24) & 0xFF, (uint8_t)(reg[SHADOW_DIAL0_IP] >> 16) & 0xFF,
            (uint8_t)(reg[SHADOW_DIAL0_IP] >>  8) & 0xFF, (uint8_t)reg[SHADOW_DIAL0_IP] & 0xFF,
            DEFAULT_NET_PORT);
        AddPayload(outputMsg);
...
void MPAudio(char *msgbuf)
{
    uint16_t outval=0;
    SPI_Transaction spiTransaction;
    bool transferOk;

    if(TXBufCtrl[0].TX_Completed!=NULL)
    {
        if(TXBufCtrl[0].TX_index>=0)
        {
            if(TXBufCtrl[0].TX_delay>0)
            {
                TXBufCtrl[0].TX_delay--;
                return;
            }
            outval +=TXBufCtrl[0].TX_Completed[TXBufCtrl[0].TX_index++];

            TXBufCtrl[0].TX_index +=TXBufCtrl[0].TX_correction;
            TXBufCtrl[0].TX_correction=0;

            TXBufCtrl[0].TX_sample_count++;

            if(TXBufCtrl[0].TX_index>=DATABLOCKSIZE)
            {
                TXBufCtrl[0].TX_index=0;
                if(TXBufCtrl[0].TX_Completed==TXBufCtrl[0].TX_Ping)
                    TXBufCtrl[0].TX_Completed=TXBufCtrl[0].TX_Pong;
                else // !=Rx_Ping
                    TXBufCtrl[0].TX_Completed=TXBufCtrl[0].TX_Ping;
            }
        }
    }
    if(TXBufCtrl[1].TX_Completed!=NULL)
    {
        if(TXBufCtrl[1].TX_index>=0)
        {
            if(TXBufCtrl[1].TX_delay>0)
            {
                TXBufCtrl[1].TX_delay--;
                return;
            }
            outval +=TXBufCtrl[1].TX_Completed[TXBufCtrl[1].TX_index++];

            TXBufCtrl[1].TX_index +=TXBufCtrl[1].TX_correction;
            TXBufCtrl[1].TX_correction=0;

            TXBufCtrl[1].TX_sample_count++;

            if(TXBufCtrl[1].TX_index>=DATABLOCKSIZE)
            {
                TXBufCtrl[1].TX_index=0;
                if(TXBufCtrl[1].TX_Completed==TXBufCtrl[1].TX_Ping)
                    TXBufCtrl[1].TX_Completed=TXBufCtrl[1].TX_Pong;
                else // !=Rx_Ping
                    TXBufCtrl[1].TX_Completed=TXBufCtrl[1].TX_Ping;
            }
        }
    }
    spiTransaction.count = 1;
    ...
    transferOk = SPI_transfer(spi3,&spiTransaction);

    if(!transferOk)
    {
        //Error in SPI  of transfer already in progress
        while(1);
    }

}

void ADCStream(void *arg0)
{
    uint16_t *source;
    char longload[sizeof(uint16_t) * DATABLOCKSIZE + msgMaxLen];
    int32_t dest_choice;
    int hdrlen;
    bool local = true;
    ...
    for(;;)
    {
        ...
        if(ADCBufCtrl.RX_Completed == ADCBufCtrl.RX_Ping)
        {
            source=ADCBufCtrl.RX_Completed;
              //dest=TxRx.TX_Ping;
              dest_choice=0;
              ADCBufCtrl.ping_count++;
        }
        else if(ADCBufCtrl.RX_Completed == ADCBufCtrl.RX_Pong)
        {
            source=ADCBufCtrl.RX_Completed;
            //dest=TxRx.TX_Pong;
            dest_choice=1;
            ADCBufCtrl.pong_count++;
        }
        else // ! RX_Ping AND ! RX_Pong is bad
        {
            errorADC++;
            AddPayload("-print Error ADC - ADC stream reading failed");
            return;
        }
        if(reg[SHADOW_DIAL0_IP]!=0)
        {
            sprintf(longload, "-netudp %d.%d.%d.%d:%d -voice %d 128  ",
        (uint8_t)(reg[SHADOW_DIAL0_IP]>>24) & 0xFF, (uint8_t)(reg[SHADOW_DIAL0_IP]>>16) & 0xFF, (uint8_t)(reg[SHADOW_DIAL0_IP]>>8) & 0xFF,
        (uint8_t)(reg[SHADOW_DIAL0_IP]) & 0xFF, DIAL_PORT0, dest_choice);
        ...
        {
            sprintf(longload, "-netudp %d.%d.%d.%d:%d -voice %d 128  ",
        (uint8_t)(reg[SHADOW_DIAL1_IP]>>24) & 0xFF, (uint8_t)(reg[SHADOW_DIAL1_IP]>>16) & 0xFF, (uint8_t)(reg[SHADOW_DIAL1_IP]>>8) & 0xFF,
        (uint8_t)(reg[SHADOW_DIAL1_IP]) & 0xFF, DIAL_PORT1, dest_choice+2);
     ...
     //   if((reg[SHADOWSHIFT_DIAL0_IP]==0) && (reg[SHADOWSHIFT_DIAL1_IP]==0)) // do local voice
        if(local==true)
        {
            sprintf(longload, "-voice %d 128  ",dest_choice + reg[0]);
            hdrlen=strlen(longload)+1;
            memcpy(&longload[hdrlen], source, sizeof(uint16_t) * DATABLOCKSIZE);
            MPvoice(longload);
        }
